...
void main() {
    ...
    float spotAtten = min(1.0, max(0.0, dot(lDir, LightDir) - SpotAngle) * SpotExponent);
    nDotL *= spotAtten;
    ...
    vec4 lightingPosition = lightViewProjection * vec4(worldPos, 1.0);
    ...
    nDotL = Filter(nDotL, shadowTexCoord, ourdepth);
