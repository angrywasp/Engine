...
public static class ReflectionMapShaderText {

public const string PixelShaderPrefiltered =
...
void main()
{
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        vec3 H  = ImportanceSampleGGX(Xi, N, Roughness / 10.0);
        vec3 L  = normalize(2.0 * dot(V, H) * H - V);

        float NdotL = max(dot(N, L), 0.0);
        if(NdotL > 0.0)
        {
            prefilteredColor += texture(AlbedoMap, L).rgb * NdotL;
            totalWeight      += NdotL;
        }
    }
