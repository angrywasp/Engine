...
namespace Microsoft.Xna.Framework
{
    public struct BoundingBox : IEquatable<BoundingBox>
    {
        ...
        public static BoundingBox Expand(BoundingBox bb, float amount)
        {
            return new BoundingBox(bb.Min - (Vector3.One * amount), bb.Max + (Vector3.One * amount));
        }

        public static BoundingBox Transform(BoundingBox bb, Matrix4x4 m)
        {
            BoundingBox ret;
            Transform(ref bb, ref m, out ret);
            return ret;
        }

        public static void Transform(ref BoundingBox src, ref Matrix4x4 transform, out BoundingBox dst)
        {
            ...
            //todo: get rid of the vector initializations

            var transformRight = transform.Right();
            var transformUp = transform.Up();
            var transformForward = transform.Forward();

            rotatedExtents.X = Math.Abs(extents.X * transformRight.X) + Math.Abs(extents.Y * transformUp.X) + Math.Abs(extents.Z * transformForward.X);
            rotatedExtents.Y = Math.Abs(extents.X * transformRight.Y) + Math.Abs(extents.Y * transformUp.Y) + Math.Abs(extents.Z * transformForward.Y);
            rotatedExtents.Z = Math.Abs(extents.X * transformRight.Z) + Math.Abs(extents.Y * transformUp.Z) + Math.Abs(extents.Z * transformForward.Z);
